...
export class Update extends plugin {
    ...
    async runUpdate(isForce) {
        const _path = './plugins/nai-plugin/'
        ...
        let time = await this.getTime('nai-plugin')

        if (/(Already up[ -]to[ -]date|已经是最新的)/.test(ret.stdout)) {
            await this.reply(`nai-plugin已经是最新版本\n最后更新时间：${time}`)
        } else {
            // 加入了一个强提醒，防止你忘了本地改过代码
            await this.reply(`nai-plugin\n最后更新时间：${time}\n⚠️ 警告：在线更新可能会覆盖你本地魔改的 Core.js 修复文件，如遇画图报错请重新修改！`)
            this.isUp = true
            let log = await this.getLog('nai-plugin')
    ...
    async gitErr(err, stdout) {
        ...
        if (errMsg.includes('be overwritten by merge')) {
            await this.reply(
                msg +
                `存在冲突：\n${errMsg}\n` +
                '请解决冲突后再更新，或者执行#nai --update强制，放弃本地修改'
            )
            return
        }
